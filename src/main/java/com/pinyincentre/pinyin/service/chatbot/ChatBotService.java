    ...
    public String getGeminiResponse(String userPrompt) {
        ...
        try {
            ...
            if (e.getStatusCode().value() == 400) {
                return "Lỗi yêu cầu không hợp lệ. Vui lòng thử câu hỏi khác.";
            }
            if (e.getStatusCode().value() == 403) {
                return "API key không có quyền truy cập. Vui lòng kiểm tra lại cấu hình.";
            }
            return "Lỗi từ Gemini API (mã " + e.getStatusCode().value() + "). Vui lòng thử lại sau.";

        } catch (Exception e) {
            logger.error("Error processing Gemini API request: {}", e.getMessage(), e);
            return "Có lỗi xảy ra trong hệ thống: " + e.getMessage();
        }
    }
